...
3. On the first run, prepares each workspace at its own path, in binding order: clones Git repos and sets up the skills path. After the sandbox reports ready, a binding with a goal starts the Antigravity agent in the background. The default Antigravity agent needs `GEMINI_API_KEY`. Set `AX_GOAL_AGENT=claude` in the Task environment to use Claude Code with `ANTHROPIC_API_KEY` from `anthropic-api-secret` in the task atespace. The selected agent gets 10 minutes by default; set `AX_BOOTSTRAP_TIMEOUT` to a Go duration to change that. The task reports workspace readiness after preparation, while the goal and task command can continue running.
